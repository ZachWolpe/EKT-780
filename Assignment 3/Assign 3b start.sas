...
* Export data;
proc export data=a dbms=csv
outfile="/folders/myfolders/sasuser.v94/EKT 720/house_data.csv"
replace;
run;
...
do i = 1 to n;
	...
	* predicted value for omitted observation;
	yho = x1[i,]*bh; 		
	
	* create matrix of results - 1 row per iteration;
	resj = resj // (bh` || r2 || yho || y1[i]);
end ;
	

ess = resj[, ncol(resj)] - resj[, ncol(resj)-1]; * errors = y - yhat;
ess = ess[##] ; * sum of squares;
r2_lo = 1 - ess/tss; 
...
call reg ;
print "Initial regression" bh r2 ;
*end initial reg ;
...
nm = {'bh1' 'bh2' 'bh3' 'bh4' 'bh5' 'r2'};
print resb[colname=bm];
itt = nrow(resb);


resbb = J(1,itt, 1/itt)*resb;
print resbb[colname=nm];

create bdat from resb[colname=nm];
append from resb;
...
proc univariate data=bdat ;
var bh1 bh2 bh3 bh4 bh5 r2 ;
histogram bh1 bh2 bh3 bh4 bh5 r2 ; 
run ;
