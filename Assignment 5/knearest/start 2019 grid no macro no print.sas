...
xyg = (xy || J(n,1,1)) //  (xyg || J(nrow(xyg),1,2)) ;
*AA print xyg ,(nrow(xyg)) ;
...
*AA print dist ;
 ...
 do i = 1 to ncol(dist) ;
	 dist_c = dist[,i] || y ; 
*AA if i <=5 then	 print i dist_c ;

	 call sort(dist_c,{1}) ;

	 dist_c = dist_c[1:k,] ;

*AA if i <=5 then 		 print "Sorted " dist_c ;

	 cnt= (dist_c[,2]=1)[+] || (dist_c[,2]=2)[+] || (dist_c[,2]=3)[+] ;
*AA if i <=5 then 		 print cnt ;
	 class = cnt[<:>] ;
	 res = res // class ;
*AA if i <=5 then 		 print class ;
 end ;
